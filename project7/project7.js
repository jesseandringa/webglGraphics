...
{	
	...
	for (let i = 0; i< springs.length; i++){
		...
		let length = p2.sub(p1).len();
		let d = p2.sub(p1).div(length);
		let kxL = stiffness* (length - springs[i].rest);
		let springForce = d.mul(kxL);
		let l = d.dot(velocities[i2].sub(velocities[i1]));
		let dampForce = d.mul(l * damping);
		
		let gravy = gravity.mul(.15);
		// console.log('kxl ' + kxL);
		// console.log('d '+ d);
		
		if (forces[i1] == undefined) forces[i1] = new Vec3(gravy.x,gravy.y,gravy.z);
		if (forces[i2] == undefined) forces[i2] = new Vec3(gravy.x,gravy.y,gravy.z);


		forces[i1].inc(springForce);
		forces[i1].inc(dampForce);
		forces[i2].inc(springForce.mul(-1));
		forces[i2].inc(dampForce.mul(-1));
		// if(kxL != 0) console.log('mmmmmm  ' + kxL +' '+springForce.x );	
	}

	// [TO-DO] Update positions and velocities
	for( let i = 0; i< positions.length; i++){
		let accel = forces[i].div(particleMass);
		console.log('velocity ' + velocities[i].x);
		velocities[i].inc(accel.mul(dt));
		console.log('velocity ' + velocities[i].x);

		positions[i].inc(velocities[i].mul(dt));
		// console.log('posi ' +positions[i].y);

	}
	// console.log('positions y'+ positions[0].y);
...
// x collision
		if (positions[i].x >=1){
			let length = positions[i].x -1; 
			let h = length * restitution;
			positions[i].x  = positions[i].x - length - h;
		...
		if (positions[i].y >=1){
			...
			let length = positions[i].y -1; 
			let h = length * restitution;
			positions[i].y  = positions[i].y - length - h;
			...
			velocities[i].y = -velocities[i].y* restitution;
			// velocities[i] = velocities[i].mul(restitution);
			// console.log('collision y post ' + positions[i].y);
			// console.log('collsions speed p[ost] ' + velocities[i].y);
			// console.log('collsions speed p[x] ' + velocities[i].x);
			// console.log('collsions speed p[z] ' + velocities[i].z);
		...
		if (positions[i].z >=1){
			...
			let length = positions[i].z -1; 
			let h = length * restitution;
			positions[i].z  = positions[i].z - length - h;
